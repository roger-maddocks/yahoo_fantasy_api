...
use crate::models::{player, report::Report, scheduled_games, team, yahoo_auth_profile};
use anyhow;
use futures::{FutureExt, TryFutureExt};
use helpers::{
    interface_helpers::{
        exit_program, get_user_command, get_user_report_bounds, user_requests_free_agents,
        user_requests_weekly_reports,
    },
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        match user_input {
            ...
            x if x == "sc" => fantasy_factory
                .await
                .get_league_stat_categories()
                .await
                .unwrap(),
            x if x == "s" => not_implemented(&x),
            x if x == "cr" => not_implemented(&x),
            x if x == "cd" => not_implemented(&x),
            x => {
                println!(
                    "Sorry but there is no functionality associated with {:?}.",
                    x
                );
            }
        }
...
async fn get_overloaded_report() {
    ...
    let report = weekly_data_factory::get_loaded_schedule_report(&FantasyWeek::new(
        get_user_report_bounds(),
    ))
    .await;
...
async fn get_my_collision_report(
    indexed_report: &mut Option<(&u64, &Report)>,
    factory: &YahooFantasyFactory,
) {
